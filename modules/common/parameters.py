...
from modules.common import convert_filter_to_camel_case
from modules.common.string_case import to_snake_case


async def common_filter_parameters(
    page: int = 1, limit: int = 100, filter: str = '{}', include: str = None, join: str = '{}',
        orderBy: str = None,
):
    if join:
    ...
    if orderBy and orderBy != '':
        ...
        orderBy = None
    return {'skip': skip, 'limit': limit, 'filter': filter_, 'include': include, 'order_by': orderBy, 'join': join_}


async def common_filter_parameters_chat_bot(
    page: int = 1, limit: int = 100, filter: str = '{}', include: str = None,
        orderBy: str = None, chatBotId: Optional[int] = None, join: str = '{}',
):
    if filter:
        filter_ = convert_filter_to_camel_case(filter)
    else:
    ...
    if orderBy and orderBy != '':
        ...
        orderBy = None
    return {'skip': skip, 'limit': limit, 'filter': filter_, 'include': include, 'order_by': orderBy, 'chat_bot_id': chatBotId, 'join': join_}
...
async def stripe_pagination(
    startingAfter=None, limit: int = 100
):
