...
from modules.common.string_case import decamelize
from datetime import datetime
from modules.common.util import clone_model
from modules.db.base_class import Base
from modules.db.query_builder import get_count, query_builder, get_filter
...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_multi(
        ...
        query = query_builder(
            db=db, model=self.model, filter=filter_param['filter'], order_by=filter_param[
                'order_by'], include=filter_param['include'], join=filter_param['join']
        )
    ...
    def get_one_or_fail(self, db: Session, id: Any) -> Optional[ModelType]:
        model = db.query(self.model).filter(
            self.model.id == id, self.model.deleted_at == None).first()
        if not model:
    ...
    def update_one_by(self, db: Session, filter: dict = {}, obj_in: Union[UpdateSchemaType, Dict[str, Any]] = '{}') -> Optional[ModelType]:
        model = db.query(self.model)\
            .filter(and_(self.model.deleted_at == None, get_filter(self.model, filter)))\
            .first()
